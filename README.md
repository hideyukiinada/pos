...
This repo contains example code for tagging words in a sentence with corresponding [part of speech] (https://en.wikipedia.org/wiki/Part_of_speech) using LSTM.  For example, parts of speech for a sentence *Dogs are awesome* is noun (for *Dogs*), verb (for *are*), adjective (for *awesome*). period (for the final .).
